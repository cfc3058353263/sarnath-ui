<template>
    <div class="app-container">
        ...
        <el-row :gutter="10" class="mb8">
            <el-col :span="1.5">
                <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd" v-hasPermi="['assets:tools:wareHouse:add']">新增</el-button>
            </el-col>
            <right-toolbar :showSearch.sync="showSearch" @queryTable="getWareHouseList"></right-toolbar>
        </el-row>
        <!-- 按钮.... -->
        <!-- table -->
        <el-table v-loading="loading" :data="list" @selection-change="handleSelectionChange" @row-click="rowClick" @current-change="handleCurrentChange" :row-class-name="tableRowClassName" :row-style="rowClass">
            <el-table-column align="center" type="index" width="55" prop="prop" label="序号" />
            <el-table-column label="入库单号" prop="orderCode" :show-overflow-tooltip="true" />
            ...
            <el-table-column label="入库时间" prop="updateTime" :show-overflow-tooltip="true">
                <template slot-scope="scope">
                    <span>{{
           scope.row.updateTime&&moment(scope.row.updateTime).format("yyyy-MM-DD HH:mm:ss")
            ...
            <el-table-column label="入库人" prop="optName" :show-overflow-tooltip="true" />
            <el-table-column label="入库状态" prop="status" :show-overflow-tooltip="true">
                <template slot-scope="scope">
            ...
            <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
                <template slot-scope="scope">
                    <el-button v-if="scope.row.status == 1" size="mini" type="text" icon="el-icon-edit" v-hasPermi="['assets:tools:wareHouse:edit']" @click="handleUpdate(scope.row)">编辑</el-button>
                    <el-button v-if="scope.row.status == 1" size="mini" type="text" icon="el-icon-circle-check" v-hasPermi="['assets:tools:wareHouse:confirm']" @click="confirm(scope.row)">确认</el-button>
                    <el-button v-if="scope.row.status == 1" size="mini" type="text" icon="el-icon-delete" v-hasPermi="['assets:tools:wareHouse:remove']" @click="handleDelete(scope.row)">删除</el-button>
                </template>
            </el-table-column>
        </el-table>
        <!-- table -->
        <!-- 分页 -->
        <pagination v-show="total > 0" :total="total" :page.sync="query.currentPage" :limit.sync="query.size" @pagination="getWareHouseList" :page-sizes="[7]" />
        <!-- 分页 -->
        <div>入库明细列表</div>
        <el-table v-loading="loading" :data="detailed" @selection-change="handleSelectionChange">
            ...
            <el-table-column label="最近一次检验时间" prop="checkTime" :show-overflow-tooltip="true" width="150">
                <template slot-scope="scope">
                    <span>{{scope.row.checkTime&&moment(scope.row.checkTime).format("yyyy-MM-DD")}}</span>
                </template>
            </el-table-column>
            <el-table-column label="下次检验时间" prop="nextCheckTime" :show-overflow-tooltip="true" width="150">
                <template slot-scope="scope">
                    <span>{{scope.row.nextCheckTime&&moment(scope.row.nextCheckTime).format("yyyy-MM-DD")}}</span>
                </template>
            </el-table-column>
            <el-table-column label="入库数量" prop="number" :show-overflow-tooltip="true" width="100" />
...
export default {
    ...
    methods: {
        ...
        getWareHouseList() {
            this.loading = true;
            wareHouseList(this.query).then((res) => {
        ...
        handleAdd() {
            this.$router.push({
                path:
                    this.$store.getters.routerPrefix + "/assets/wareHouse/data",
            });
        },
        /** 编辑按钮操作 */
        handleUpdate(row) {
