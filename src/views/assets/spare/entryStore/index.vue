...
import {OrdersIn} from "@/api/assets/tools/wareHouse";
import {userSelectList} from "@/api/system/user";

export default {
  name: "SpareEntryStore",
  data() {
    let self = this;
    return {
      endTime: {
  ...
  methods: {
    ...
    getList() {
      ...
      getSpareOrderByPage(this.query).then((respons) => {
    ...
    handleAdd() {
      this.$router.push({
        path: this.$store.getters.routerPrefix + '/assets/spare/entryStore/data',
      });
    },
    /**
     * 点击编辑按钮操作
