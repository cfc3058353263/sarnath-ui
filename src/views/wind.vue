                        ...
                        <el-row style="margin-right:5px" :gutter="10">
                            <el-col :xs="24" :sm="12" :lg="8" v-for="(item,index) of stationList" :key="index" style="margin-top:10px">
                                <transition name="el-zoom-in-top">
                                    <div class="stationitem" @click="windFieldPush(item)" v-show="show">
                                        ...
                                        <div><span class="itemTitle">装机容量：{{item.installCapacity}}KW</span></div>
                                        <div><span class="itemTitle">电站状态：{{item.stationStatus==1?"运行中":"维修中"}}</span></div>
                                        <div><span class="itemTitle">并网时间：{{moment(item.netTime).format("YYYY-MM-DD")}}</span></div>
            ...
            <el-col :span="10">
                ...
                <div class="monitor">
                    <div class="item1" style="color:#088165">
                        <div><span class="itemTitle1">{{data.day_kwh}}</span>kWh</div>
                        <div style="margin-top:10px">昨日电量</div>
                    </div>
                    <div class="item1" style="color:#088165">
                        <div><span class="itemTitle1">{{data.momthRatio}}</span>%</div>
                        <div style="margin-top:10px">月完成比</div>
                    </div>
                    <div class="item2" style="color:#088165">
                        <div><span class="itemTitle1">{{data.yearRatio}}</span>%</div>
                ...
                <div class="monitor">
                    <div class="item1" style="color:#9F479B">
                        <div><span class="itemTitle1">{{data.dayLoss}}</span>kWh</div>
                        <div style="margin-top:10px;">昨日损失</div>
                    </div>
                    <div class="item1" style="color:#9F479B">
                        <div><span class="itemTitle1">{{data.momthLoss}}</span>kWh</div>
                        <div style="margin-top:10px;">月损失</div>
                    </div>
                    <div class="item2" style="color:#9F479B">
                        <div><span class="itemTitle1">{{data.yearLoss}}</span>kWh</div>
                 ...
                 <div class="monitor">
                    <div class="item1" style="color:#395EAB">
                        <div><span class="itemTitle1">{{data.clzOrder}}</span>个</div>
                        <div style="margin-top:10px;">处理中工单</div>
                    </div>
                    <div class="item1" style="color:#395EAB">
                        <div><span class="itemTitle1">{{data.addOrder}}</span>个</div>
                        <div style="margin-top:10px;">日增工单</div>
                    </div>
                    <div class="item2" style="color:#395EAB">
                        <div><span class="itemTitle1">{{data.monthOrder}}</span>个</div>
...
export default {
    ...
    methods: {
        ...
        windFieldPush(item) {
            ...
            this.$router.push({ path: "/wind/index" }).catch((err) => {});
        },
        search() {
